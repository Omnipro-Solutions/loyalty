 ...
 * Preguntar o cualquier chip abre el panel con la conversación de ejemplo
 * de `mock-data.ts` — no hay un modelo real detrás todavía.
 */
export function AiCopilotHero({ name }: AiCopilotHeroProps) {
  const [isChatOpen, setIsChatOpen] = useState(false)

  function handleKeyDown(event: KeyboardEvent<HTMLInputElement>) {
    if (event.key === "Enter") setIsChatOpen(true)
  }
